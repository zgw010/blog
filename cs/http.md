...
再补充一个 [HSTS](https://zh.wikipedia.org/wiki/HTTP%E4%B8%A5%E6%A0%BC%E4%BC%A0%E8%BE%93%E5%AE%89%E5%85%A8)
# HTTP/2
...
[HTTP/2的起源, SPDY](https://developers.google.com/web/fundamentals/performance/http2/?hl=zh-cn)

HTTP/1.x的不足:

HTTP实现简单是以牺牲应用性能为代价的： HTTP/1.x 客户端需要使用多个连接才能实现并发和缩短延迟；HTTP/1.x 不会压缩请求和响应标头，从而导致不必要的网络流量；HTTP/1.x 不支持有效的资源优先级，致使底层 TCP 连接的利用率低下
...
补充知识: [http 最多可以对同一个域名建立6个tcp连接](https://hpbn.co/http1x/#domain-sharding)

[TCP 的慢启动, 拥塞控制, 快速重传, 快速恢复](https://blog.csdn.net/itmacar/article/details/12278769), 这篇文章讲的有点问题, 不过大致没毛病, 之后再看的时候, 看计算机网络(自顶向下)的书, 比较清晰.
